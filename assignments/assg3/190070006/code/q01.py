...
class DynamicArray:
    ...
    def shrink_array(self):
        ...
        B = self._make_array(int((self._capacity)/2))  # new (bigger) array
        for k in range(int((self._capacity)/2)):  # for each existing value
            B[k] = self._A[k]
        self._A = B  # use the bigger array
    ...
    def remove(self, value):
        ...
        for k in range(self._n):
            if self._A[k] == value:  # found a match!
                ...
                """
                if (self._n == self._capacity/2 and self._capacity%2==0):
